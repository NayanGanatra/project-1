...

{
...

	{
		...
		$sql = "SELECT a.* FROM ".$this->config->item('convention_db_prefix')."program_member a  WHERE a.pm_id IN (SELECT pm_id FROM ".$this->config->item('convention_db_prefix')."program_to_participant WHERE pm_member_name like '%".$search_key."%') OR a.pm_mm_id IN (SELECT mm_id from member_master WHERE mm_fname like '%".$search_key."%' OR mm_lname like '%".$search_key."%') OR (a.pm_type like '%".$search_key."%'
...

	{
		...
		if($this->db->update($this->config->item('convention_db_prefix').'program_member', $data))
...

	{
		...
		$this->db->delete($this->config->item('convention_db_prefix').'program_member', array('pm_id' => $id));
...

	{
		...
		$sql="SELECT * FROM ".$this->config->item('convention_db_prefix')."program_to_participant WHERE pm_id = '$pm_id' ORDER BY pm_to_participant_id DESC";
...

	{
		...
		$this->db->delete($this->config->item('convention_db_prefix').'program_to_participant', array('pm_id' => $id));
...

	{
		...
		$sql="SELECT * FROM ".$this->config->item('convention_db_prefix')."program_member WHERE pm_id='".$id."' LIMIT 1";
	...
	{	
		...
		$sql="SELECT * FROM ".$this->config->item('convention_db_prefix')."settings";
