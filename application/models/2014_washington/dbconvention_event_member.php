...
class Dbconvention_event_member extends CI_Model
{
	...
	function get_all_event_member($num,$offset)
	{
		...
		//$sql="SELECT * FROM ".$this->config->item('convention_db_prefix')."events_member ORDER BY ce_mem_id DESC";
		$query = $this->db->query($sql);
		return $query->result();
	}
	function get_user_status()
	{
		$sql="SELECT * FROM ".$this->config->item('convention_db_prefix')."events_member WHERE ".$this->config->item('convention_db_prefix')."mm_id='".$this->session->userdata('user_id')."'";
		$query = $this->db->query($sql);
		return $query->num_rows();
	}
	...
	function edit_con_events_member_group($data,$ce_mem_id,$ce_id)
	{
		...
		$this->db->where($this->config->item('convention_db_prefix')."mm_id", $ce_mem_id);
	...
	function get_user_status_of_con_event()
	{
		...
		$sql="SELECT * FROM ".$this->config->item('convention_db_prefix')."events_member WHERE con_mm_id='".$this->session->userdata('user_id')."' LIMIT 1";
		$query = $this->db->query($sql);
		return $query->row();
	}
	//update end
	function ce_age_desc($id)
	{
		...
		$sql="SELECT ce_age_desc FROM ".$this->config->item('convention_db_prefix')."events where ce_id=".$id;
		$query = $this->db->query($sql);
		return $query->row();
	}
}
