...
function __construct()
	...
	{
		$get_rsvp = $this->dbheader->get_rsvp($event_id);
		...
		if($get_rsvp)
		{
			...
			echo '<th>Adults</th>';
			echo '<th>Kids</th>';
			echo '<th>RSVP Date &amp; Time</th>';
			echo '</tr>';
			foreach($get_rsvp as $get_rsvp_row)
			{
				echo '<tr><td>'.$get_rsvp_row->mm_fname.' '.$get_rsvp_row->mm_lname.'</td>';
				echo '<td>'.$get_rsvp_row->mm_email.'</td>';
				echo '<td>'.$get_rsvp_row->adult_count.'</td>';
				echo '<td>'.$get_rsvp_row->kids_count.'</td>';
				echo '<td>'.date('D, d M Y H:i:s',strtotime($get_rsvp_row->rsvp_time)).'</td></tr>';
			}
			echo '</table>';
		}
		else
		{
			echo '<h3 style="padding:20px;">No data found!</h3>';
		}
	}
	
	function get_invitation($event_id)
	{
		$get_invitation = $this->dbheader->get_invitation($event_id);
		...
		if($get_invitation)
		{
			...
			echo '<th>Phone</th>';
			echo '<th>Mobile</th>';
			echo '<th>Sent Date &amp; Time</th>';
			echo '</tr>';
			foreach($get_invitation as $get_invitation_row)
			{
				echo '<tr><td>'.$get_invitation_row->mm_fname.' '.$get_invitation_row->mm_lname.'</td>';
				echo '<td>'.$get_invitation_row->mm_email.'</td>';
				echo '<td>'.$get_invitation_row->mm_hphone.'</td>';
				echo '<td>'.$get_invitation_row->mm_cphone.'</td>';
				echo '<td>'.date('D, d M Y H:i:s',strtotime($get_invitation_row->create_time)).'</td></tr>';
			}
			echo '</table>';
		}
		else
	...
	{
		$get_rsvp = $this->dbheader->get_people($event_id);
	...
	{
		$get_rsvp = $this->dbheader->get_maybe($event_id);
	...
	{
		$get_rsvp = $this->dbheader->get_notcoming($event_id);
	...
	{
		$get_rsvp = $this->dbheader->get_pending($event_id);
	...
	function delete_page()
	{
		...
		if($data['page'])
		{
			$this->dbca->delete_page($this->uri->segment(3));
			$this->session->set_flashdata('message_type', 'success');
			$this->session->set_flashdata('status_', 'Page successfully deleted');
			redirect(base_url().'ca/pages');
		}
	}
	...
	public function users()
	{
		...
		$num_rec=$this->dbca->count_all_ch_user($data['user']->mm_chapter_id);
		$data['tot_rec'] = $num_rec;
		
		$this->load->library('pagination');
		
		$config['base_url'] = base_url().'ca/users/';
		$config['total_rows'] = $num_rec;
		$config['per_page'] = 40;
		...
		$data['query'] = $this->dbca->get_all_ch_user($data['user']->mm_chapter_id,$config['per_page'],$segment);
		$this->pagination->initialize($config);
		$data['sub_title'] = 'Total Members: '.$num_rec;
		$data['title']="Manage Members";		
		$this->load->view('ca_users',$data);
	}
	...
	public function search_users()
	{
		...
		$num_rec=$this->dbca->count_all_ch_user_search($data['user']->mm_chapter_id,$this->input->get('search'));
		$data['tot_rec'] = $num_rec;
		...
		$data['query'] = $this->dbca->get_all_ch_user_search($data['user']->mm_chapter_id,$this->input->get('search'),$config['per_page'],$segment);
		$this->pagination->initialize($config);
		$data['sub_title'] = 'Total Members: '.$num_rec;
		$data['title']="Manage Members";		
		$this->load->view('ca_users',$data);
	...
	function edit_user($id)
	{
		...
		$data['chapter'] = $this->dbchapter->chapter_byid($user->mm_chapter_id);
		
		$data['user'] = $this->dbca->get_user($user->mm_chapter_id,$id);
		
		if(!$data['user'])
		{
			$this->session->set_flashdata('message_type', 'error');
			$this->session->set_flashdata('status_', 'User not exist or you do not have permission to edit this user!');
			redirect(base_url().'ca/users.html');
		}
		...
		else
		{
			...
			$data['description'] = "";
			$data['keywords'] = "";
			$this->load->view('ca_edit_users',$data);
	}
	function delete_user($id)
	{
		...
		else
		{
			...
			$this->session->set_flashdata('message_type', 'success');
			$this->session->set_flashdata('status_', 'User deleted successfully!');
			redirect(base_url().'ca/users.html');
		}
	}
	...
	function login_history()
	{
		...
		$num_rec=$this->dbca->count_all_ch_history($data['user']->mm_chapter_id);
		$data['tot_rec'] = $num_rec;
		
		$this->load->library('pagination');
		
		$config['base_url'] = base_url().'ca/login_history/';
		$config['total_rows'] = $num_rec;
		$config['per_page'] = 40;
		$config['uri_segment'] = 3;
		$config['full_tag_open'] = '<div align="center" class="pagination"><ul>';
		...
		$this->pagination->initialize($config);
		$data['query'] = $this->dbca->get_all_ch_history($data['user']->mm_chapter_id,$config['per_page'],$segment);
		
		$data['title'] = 'Member Login History';
		$data['sub_title'] = '';
		$data['description'] = "";
		$data['keywords'] = "";
		$this->load->view('ca_login_hostory',$data);
	}
}