...
public function destroy()
	...
	$string.= "<table id='ReportTable' cellpadding='2' cellspacing='2' border='1' bordercolor='#d0d7e5'>";
		$fp = fopen($this->config->item('rootfolderpath').'excel_files/'.$filename, 'w');
	...
	public function view()
	{
		...
		else
		{
			...
			if($member_type == '1')
			{
				$num_rec=$this->dbuser->count_user_search_assign_final($member_chapter,$member_search);
			}
			if($member_type == '2')
			{
		...
		if($member_type == '1')
		{
			$data['query'] = $this->dbuser->get_all_user_search_assign_final($member_chapter,$member_search,$config['per_page'],$segment);
		}
		if($member_type == '2')
		{
