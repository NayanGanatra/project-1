...
class Dbemail extends CI_Model
{
	...
	*/
	function count_email()
	{
		...
		$sql="SELECT b.* FROM email a, chapter_to_email_template b WHERE a.uid = b.email_template_id AND b.ch_id = '".$this->session->userdata('get_chapter_id')."'";
		$query = $this->db->query($sql);
	...
	function get_all_email($num,$offset)
	{
		...
		$sql="SELECT a.* FROM  email a,chapter_to_email_template b WHERE  a.uid = b.email_template_id AND b.ch_id = '".$this->session->userdata('get_chapter_id')."' ORDER BY a.uid DESC LIMIT ".$offset.", ".$num."";
...
$sql="SELECT b.mm_id FROM email a 
INNER JOIN email_template_to_member b ON a.uid=b.email_template_id
