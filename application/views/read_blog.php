...
<div class="span10 nomargin" style="width:740px;">
<?php if(isset($this->session->userdata['user_id'])){ ?>
		<?php if($blog){ 
		?>
        <div class="page_content" style="min-height:50px;">
	...
	<span style="float:left; margin-left: 10px; width:92%;"><h4 class="nopadding nomargin bold" style="color:#0088CC;">
		<?php echo $blog->title; ?>
	</h4></span>
...
<div id="comments">
        ...
        <?php
			$commentCount = $this->dbchapter->get_all_comments_count($blog->bid);
			$replyCount = $this->dbchapter->get_all_reply_count($blog->bid);
			?>
			...
			<?php
			$query = $this->dbchapter->get_all_comments($blog->bid,10,0);
			if($query)
			{
				...
				{?>
					...
					<p style="margin: 25px 0 0 0;"><?php echo $row->text; ?></p>
					
					<a onclick="reply_toggle(this);" href="javascript: void(0)" class="reply" id="<?php echo $row->comment_id; ?>">reply</a><br />
					</div>
					<!--<span><?php echo 'comment: By ';?><b><?php echo $commentUser->mm_username; ?></b> <i><?php echo $row->text; ?></i> <?php echo $row->comment_date; ?></span>
					
					<a onclick="reply_toggle(this);" href="javascript: void(0)" class="reply" id="<?php echo $row->comment_id; ?>">reply</a><br />-->
					<?php
						$queryReply = $this->dbchapter->get_all_reply($row->comment_id,$blog->bid);
						if($queryReply)
						{
					...
					?>
					<form class="hideall" id="submitreply<?php echo $row->comment_id; ?>" action="<?php echo base_url().'blog/read_blog/'.$method.'/'.$blog->bid; ?>" method="POST" style="margin:10px 0 0 45px;">
		...
		<form id="submitcomment<?php echo $blog->bid; ?>" action="<?php echo base_url().'blog/read_blog/'.$method.'/'.$blog->bid; ?>" method="POST" style="margin:10px 0 0 0px;">
...
$(document).ready(function() {
		...
		$.ajax({
	   type: "POST", 
	   data: "id="+<?php echo $blog->bid; ?>+"&limit="+$('#hdnstart').val()+"&offset="+$('#hdnend').val()+"&method="+'<?php echo $method; ?>',
	url: BASE_URI+"blog/loadmore_comments",
