...

{
...

	{
		...
		$this->load->model('admincp_convention/dbsponsors');
...

	{
		...
		else
		{
			...
			// Image upload start
				...
				// file_put_contents($this->config->item('rootfolderpath').'images/media/thumbs/'."$name", file_get_contents($url));
...

						{
							...
							if (!$this->upload->do_upload('image'))
							{
								if($this->input->post('old_image')=='')
								{
									...
									redirect(base_url().'admincp_convention/sponsors/edit/'.$id);
								}
								else
								{
					...
					redirect(base_url().'admincp_convention/sponsors');
		...
		$this->load->view('admincp_convention/sponsors/edit',$data);
	...
	public function add()
	{
				...
				->save($this->config->item('rootfolderpath')."images/convention/sponsors/"."$filename",$overwrite=TRUE);
...

					{
...

						{
							...
							//$this->session->set_flashdata('message_type', 'error');
							//$this->session->set_flashdata('status_', 'Image not uploaded, please try again');	
							//redirect(base_url().'admincp_convention/sponsors/add');
					...
					$data=array(
						...
						//'cs_ch_id'=>$this->input->post('cs_ch_id'),
						'cs_created_by'=>$this->input->post('ads_created_by'),
						'cs_modified_date'=>$this->input->post('ads_created_date'),
						'cs_modified_by'=>$this->input->post('ads_created_by'),
		...
		$this->load->view('admincp_convention/sponsors/add',$data);
	}
	public function view()
	{
		$num_rec=$this->dbsponsors->count_sponsors();
		$this->load->library('pagination');
		$config['base_url'] = base_url().'admincp_convention/sponsors/view/';
		...
		$this->load->view('admincp_convention/sponsors/view',$data);
...

	{
		...
		redirect(base_url().'admincp_convention/sponsors/view');
