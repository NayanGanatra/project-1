...
<table border="0" cellspacing="0" cellpadding="0" align="center" style="min-height:300px; float:left"> 
  ...
  <tr>
  	...
  	<td>
    	...
    	<div class="dotted_line">
		    ...
		    <h1>Verification <?php echo $this->lang->line('text_email');?> <small>Edit</small></h1>
 ...
 <b>User Info:</b>{userinfo}<br />
    ...
    <span id="fade1" class="black_overlay"></span>
			 ...
			 $mm_id_edit = $this->dbadminheader->fetch_user_for_edit_verification($item->uid);
...

			 {
				...
				$ch_to_template_id = $this->dbadminheader->verification_template_to_mm($item->uid,$user_details->mm_id);
...
Use Send button from list page to start sending emails.';?>"  >
 ...
 <div class="space20px"></div>   
            ...
            <input type="hidden" id="" name="verification_created_date" value="<?php echo $item->verification_created_date; ?>">

            <input type="hidden" id="" name="verification_created_by" value="<?php echo $item->verification_created_by; ?>">

            <input type="hidden" id="" name="verification_modified_date" value="<?php echo date("Y-m-d H:i:s"); ?>">
			...
			?>
        			...
        			<input type="hidden" id="" name="verification_modified_by" value="<?php echo $username->mm_username;?>">
...

{
...

	{
...

		{
			...
			//url: BASE_URI+"chapteradmincp/verification/user_check_uncheck_all/edit/"+uid+"/"+n_ch+"/true/"+type+"/"+$('#offset').val(),

			url: BASE_URI+"chapteradmincp/verification/user_check_uncheck_all/",

				data :'status=edit&id='+uid+'&ch_id='+n_ch+'&action=true&type='+type+'&offeset='+$('#offset').val()+'&m_type='+$('#mm_type').val()+'&m_search='+$('#username').val()+'&fetch_user_data='+fetch_user_data+'&just_use='+just_use+'&fetch_user_data_after_save='+fetch_user_data_after_save,
...
$('div#custom_box').removeClass("custom_box_css");
...

	{
...

		{
			...
			//url: BASE_URI+"chapteradmincp/verification/user_check_uncheck_all/edit/"+uid+"/"+n_ch+"/false/"+type+"/"+$('#offset').val(),

			url: BASE_URI+"chapteradmincp/verification/user_check_uncheck_all/",

			data :'status=edit&id='+uid+'&ch_id='+n_ch+'&action=false&type='+type+'&offeset='+$('#offset').val()+'&m_type='+$('#mm_type').val()+'&m_search='+$('#username').val()+'&fetch_user_data='+fetch_user_data+'&just_use='+just_use+'&fetch_user_data_after_save='+fetch_user_data_after_save,
...
$('#email_template_name').change(function() {
	...
	$.ajax({
	  ...
	  url: BASE_URI+'chapteradmincp/verification/get_template_chapter/'+$('#email_template_name').val(),
...
var mm_type=document.getElementById('mm_type').value;
			...
			url: BASE_URI+"chapteradmincp/verification/get_template_user/",
...
$('div#custom_box').addClass("custom_box_css");
	...
	$.ajax({
	  ...
	  url: BASE_URI+'chapteradmincp/verification/check_uncheck_all_user/'+$('#check_uncheck_all_user_status').val(),
...
$('div#custom_box').addClass("custom_box_css");
	...
	$.ajax({
	  ...
	  url: BASE_URI+'chapteradmincp/verification/check_uncheck_all_user/'+$('#chkall_check').val(),
	...
	//	alert(id);
		...
		$.ajax({
		 ...
		 url: BASE_URI+"chapteradmincp/verification/get_template_user/",
		   ...
		   data :'mm_type='+mm_type+'&username='+username+'&id='+<?php echo $item->uid; ?>+'&ch_id='+ n+'&offset='+0+'&fetch_user_data='+fetch_user_data+'&check_uncheck_all_user_status='+$('#check_uncheck_all_user_status').val()+'&user_count='+user_count+'&just_use='+just_use+'&fetch_user_data_after_save='+fetch_user_data_after_save+'&edit='+edit,
...
$('div#custom_box').addClass("custom_box_css");
	...
	url: BASE_URI+"chapteradmincp/verification/user_check_uncheck/edit/"+uid+"/"+m_id+"/"+action,
...
function save_user_data()
{
	...
	for (var i = 0, l = ln_user; i < l; i++) {
		...
		$.ajax({
		  ...
		  url: BASE_URI+"chapteradmincp/verification/edit_user/<?php echo $item->uid; ?>/"+n_chapter+"/"+j_user,
...
$('div#custom_box').addClass("custom_box_css");
	...
	$.ajax({
		   ...
		   url: BASE_URI+"chapteradmincp/verification/email_info/",
