...

{
	...
	function get_template_user()
	{
		...
		foreach($user_details as $user_details)
		{
			...
			?>
                 <td>
                 	<?php
					if($ch_to_template_id->mail_send_status==1 && $mm_type!=6)
					{?>
                    <span class="span3" >
	...
	function user_check_uncheck_all()
	{
		//$status,$id,$ch_id,$action,$type,$offeset;
			...
			if($action=='true')
			{
				echo $this->get_template_user_check_all($id,$ch_id,$type,$offeset,$mm_type);
			}
			else
			{
				if($type=='chkall')
	...
	function get_template_user_check_all($id,$ch_id,$type,$offeset,$mm_type)
	{
