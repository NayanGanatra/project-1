    ...
    <span id="fade1" class="black_overlay"></span>
      ...
      <div class="space20px"></div>
            ...
            <label class="control-label">Queued Newsletter</label>
                ...
                <div  style="width:160px; height:26px"class="controls" data-tip1="<?php  echo 'First Option Only Save Data. Second Option For Send Data';?>"  >
            ...
            <?php 
				...
				{
			?>
        			<input type="hidden" id="" name="newsletter_modified_by" value="<?php echo $username->mm_username;?>">
        	<?php
                }
       	 	?>
...
function chkalluser(uid,type)
{
	...
	if(status=='yes')
	{
		...
		if(id!='')
		{
			...
			$.ajax({
				   type: "POST",
			url: BASE_URI+"chapteradmincp/user/user_check_uncheck_all/add/"+uid+"/true/"+type+"/"+$('#offset').val(),
				success: function(data) {
					if(type!='chkall')
					{
...
$('div#custom_box').removeClass("custom_box_css");
	...
	else
	{
		...
		if(id!='')
		{
			...
			url: BASE_URI+"chapteradmincp/user/user_check_uncheck_all/add/"+uid+"/false/"+type+"/"+$('#offset').val(),
				success: function(data) {
...
$('#email_template_name').change(function() {
	$.ajax({
	  url: BASE_URI+'chapteradmincp/user/get_template_chapter/'+$('#email_template_name').val(),
	  success: function(data) {
		$('.nicEdit-main').html(data);
		$('#html').html(data);
		}
...
$('#cnt_user').click(function() {
	...
	$.ajax({
		...
		//data: "fetch_user_data="+fetch_user_data,
	   	//url: BASE_URI+"chapteradmincp/user/get_template_user/"+username+"/0/"+<?php echo $item->uid; ?>+"/0/"+$('#check_uncheck_all_user_status').val(),
	 	//url: BASE_URI+"chapteradmincp/user/get_template_user/"+username+"/0/"+<?php echo $item->uid; ?>+"/0/"+fetch_user_data+"/"+$('#check_uncheck_all_user_status').val(),
	   data: "fetch_user_data="+fetch_user_data+"&username="+username+"&id="+'0'+"&nId="+'<?php echo $item->uid; ?>'+"&offset="+'0'+"&check_uncheck_all_user_status="+$('#check_uncheck_all_user_status').val(),
		url: BASE_URI+"chapteradmincp/user/get_template_user/",
...
function user_check_uncheck(uid,m_id,action)
{
	...
	$.ajax({
				   type: "POST",
	url: BASE_URI+"chapteradmincp/user/user_check_uncheck/edit/"+uid+"/"+m_id+"/"+action,
	success: function(data) {
		if(action==true)
		{
...
$('#save_user_data').click(function() {
	...
	for (var i = 0, l = ln_user; i < l; i++) {
		...
		$.ajax({
				   type: "POST",
		  url: BASE_URI+"chapteradmincp/user/edit_user",
		  success: function(data) {
...
function chkall_d()
{
	...
	$.ajax({
		type: "POST",
	 	url: BASE_URI+'chapteradmincp/user/check_uncheck_all_user/'+$('#chkall_check').val(),
	  	success: function(data) {
		  var str=data.split("|");
...
$('#check_uncheck_all_user').click(function() {
	...
	$.ajax({
				   type: "POST",
	  url: BASE_URI+'chapteradmincp/user/check_uncheck_all_user/'+$('#check_uncheck_all_user_status').val(),
	  success: function(data) {
		  var str=data.split("|");
...
$('#email_preview').click(function() {
	...
	$.ajax({
		   type: "POST",
		   url: BASE_URI+"chapteradmincp/user/email_info/",
		   //$('#html').html(data);
