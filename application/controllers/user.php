...
function __construct()
	...
	function login()
	{
		...
		else
		{
			...
			//update pradip_20131127 -----------
			if($user->mm_status == 1)
			{
				$this->session->set_userdata('user_email',$user->mm_email);
				$this->session->set_userdata('mm_chapter_id',$user->mm_chapter_id);
				//added by ketan 201309271100
				...
				if($this->session->userdata('ref_url'))
				{
					if($this->uri->segment(3)=='youthmilan')
					{
						redirect(base_url()."youth_milan/registration/view");
					}
					if($this->uri->segment(3)=='convention_login')
					{
						redirect(base_url()."2015/texas/convention/welcome.html");
					}
					if($this->uri->segment(3)=='convention_registration')
					{
						redirect(base_url()."2015/texas/convention/registration.html");
					}
					if($this->uri->segment(3)=='convention_event')
					{
						redirect(base_url()."2015/texas/convention/eventmembership.html");
					}
					if($this->uri->segment(3)=='convention_program')
					{
						redirect(base_url()."2015/texas/convention/program.html");
					}
					if($this->uri->segment(3)=='convention_medical')
					{
						redirect(base_url()."2015/texas/convention/medical.html");
					}
					...
					{
						$this->session->set_flashdata('message_type', 'error');
						$this->session->set_flashdata('status_', 'please select a state');
						redirect(base_url('user/edit_profile.html'));
					   // return FALSE;
					}
					redirect(''.base_url().'user/account.html');
				}
	...
	function checkuserreg()
	{
		...
		/*if(!$this->session->userdata('ref_url'))
		{
			$this->session->set_userdata('ref_url','http://www.spcsusa.org/2015/texas/convention/eventmembership.html');
		}*/
		
		$this->form_validation->set_rules('mm_email', $this->lang->line('text_email'), 'required|trim|max_length[99]|xss_clean');
		$this->form_validation->set_rules('mm_username', $this->lang->line('text_password'), 'required|trim|max_length[200]|xss_clean|callback__checkUsername');
		...
		{
			$user = $this->dbheader->check_user($this->input->post('mm_email'),$this->input->post('mm_username'));
			//update pradip_20131127 -----------
			if($user->fm_reg_num == $this->input->post('mm_username'))
			{
				
				$this->session->set_userdata('user_event',true);
				/*$this->session->set_userdata('user_email',$user->fm_email);
				//$this->session->set_userdata('mm_chapter_id',$user->mm_chapter_id);
				//added by ketan 201309271100
				$this->session->set_userdata('id',$user->mm_id);
				//update end
		...
		$data['description'] = "";
		$data['keywords'] = "";*/
		$this->load->view('check_reg_num',$data);
	}
	...
	function _checkUsername() {
        ...
        $email = $this->input->post('mm_email');
        $password = $this->input->post('mm_username');

        $user = $this->dbheader->check_user($email,$password);

        if(! $user)
        {
            $this->form_validation->set_message('_checkUsername', $this->lang->line('text_incorrect_login'));
            return FALSE;
        }else{
            return TRUE;
        }   
	...
	function needtochange()
	{
		...
		$this->form_validation->set_rules('mm_email', $this->lang->line('text_email'), 'required|valid_email|max_length[40]|min_length[3]|xss_clean|callback__checkEditEmail');
