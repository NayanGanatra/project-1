...

{
	...
	{
		$sql="SELECT * FROM ".$this->config->item('convention_db_prefix')."fees_member WHERE mm_id='".$this->session->userdata('user_id')."'";
		$query = $this->db->query($sql);
		return $query->num_rows();
	}
	function get_user_status_for_program()
	{
		$sql="SELECT * FROM ".$this->config->item('convention_db_prefix')."program_member WHERE pm_mm_id='".$this->session->userdata('user_id')."'";
		$query = $this->db->query($sql);
		return $query->num_rows();
	}
	function get_user_status_for_medical()
	{
		$sql="SELECT * FROM   ".$this->config->item('convention_db_prefix')."medical_form WHERE md_mm_id='".$this->session->userdata('user_id')."'";
		$query = $this->db->query($sql);
		return $query->num_rows();
	}
	...
	{
		$sql="SELECT * FROM ".$this->config->item('convention_db_prefix')."fees_structure LIMIT 1";
		$query = $this->db->query($sql);
		return $query->row();
	}
...
function get_user_status_of_con_fees()
	{
		$sql="SELECT * FROM ".$this->config->item('convention_db_prefix')."fees_member WHERE mm_id='".$this->session->userdata('user_id')."' LIMIT 1";
		$query = $this->db->query($sql);
		return $query->row();
	}
...
function edit_member($data,$id)
	...
	{
		$this->db->delete($this->config->item('convention_db_prefix').'fees_member_group', array('fm_id' => $up_id));
		return true;
	}
	...
	function delete_program_registration($mm_id)
	{
		...
		$this->db->delete($this->config->item('convention_db_prefix').'program_member', array('pm_mm_id' => $mm_id));
		return true;
	}
	
	function delete_medical_registration($mm_id)
	{
		$this->db->delete($this->config->item('convention_db_prefix').'medical_form', array('md_mm_id' => $mm_id));
		return true;
	}
	
	function delete_program_participant_registration($pm_id)
	{
		$this->db->delete($this->config->item('convention_db_prefix').'program_to_participant', array('pm_id' => $pm_id));
		return true;
	}
	
	function delete_event_registration($mm_id)
	{
		$this->db->delete($this->config->item('convention_db_prefix').'events_member', array('con_mm_id' => $mm_id));
		$this->db->delete($this->config->item('convention_db_prefix').'events_member_group', array('con_mm_id' => $mm_id));
		return true;
	}
	...
	{
		$sql="SELECT * FROM ".$this->config->item('convention_db_prefix')."program_member WHERE pm_mm_id='".$mm_id."' LIMIT 1";
		$query = $this->db->query($sql);
		return $query->result();
	}
