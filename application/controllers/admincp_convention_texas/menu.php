...

{
...

	{
		...
		$this->load->model('admincp_convention_texas/dbmenu');
...

	{
		...
		$config['base_url'] = base_url().'admincp_convention_texas/menu/view/';
		...
		$this->load->view('admincp_convention_texas/menu/view',$data);
...

	{
...

		{
						...
						if($this->input->post('menu_type') == '0')
						{
							...
							if($this->input->post('menu_int_link') == base_url().$this->config->item('convention_texas_folder_with_slash').'convention/pages')
							{
								 $menu_link =$this->input->post('int_page');
                                
							}
							elseif($this->input->post('menu_int_link') == base_url().$this->config->item('convention_texas_folder_with_slash').'convention/sponsors')
							{
			...
			$data=array(
						...
						for($b=0; $b < count($sub_menu_name); $b++)
							...
							{					
									...
									if($sub_menu_type[$b] == '0')
									{
										...
										if($sub_menu_link[$b] == base_url().$this->config->item('convention_texas_folder_with_slash').'convention/pages')
										{
											
											$sub_menu_link = $_POST['sub_int_pages'];
										}
										elseif($sub_menu_link[$b] == base_url().$this->config->item('convention_texas_folder_with_slash').'convention/sponsors')
										{
					...
					redirect(base_url().'admincp_convention_texas/menu');
		...
		$this->load->view('admincp_convention_texas/menu/edit',$data);
	...
	public function add()
	{
					...
					if($this->input->post('menu_type') == '0')
						{
							...
							if($this->input->post('menu_int_link') == base_url().$this->config->item('convention_texas_folder_with_slash').'convention/pages')
							{
								echo $menu_link =$this->input->post('int_page');
						...
						{
							echo $menu_link =$this->input->post('menu_ext_link');
		...
		$data['title']="Add Menu";
		$this->load->view('admincp_convention_texas/menu/add',$data);
	}
...

	{
		...
		redirect(base_url().'admincp_convention_texas/menu/view');
