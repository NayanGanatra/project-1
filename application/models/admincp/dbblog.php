...
class Dbblog extends CI_Model
{
	...
	{
		$sql = "SELECT bid FROM blog WHERE by_mem=0 ";
		$query = $this->db->query($sql);
		return $query->num_rows();
	}
	function get_all_blogs($num,$offset)
	{
		//$sql="SELECT a.* FROM blog a, member_to_blogs b WHERE  a.bid != b.blog_id ORDER BY bid DESC LIMIT ".$offset.", ".$num."";
		//$sql="SELECT * FROM blog WHERE by_mem=0 ORDER BY bid DESC LIMIT ".$offset.", ".$num."";
		$sql="SELECT * FROM blog ORDER BY bid DESC LIMIT ".$offset.", ".$num."";
		$query = $this->db->query($sql);
		return $query->result();
	}
