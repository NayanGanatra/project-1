...
class Email extends CI_Controller {
...

	{
		...
		//$check_uncheck_all_user_status = "no";
		...
		$state = substr($state, 0,-1);
				...
				$user_details_data= $this->dbadminheader->user_details_state_for_email($state);
							...
							if($fetch_user_data=="" || $fetch_user_data=='0')
							{
								...
								
								{
									if(isset($_POST['edit']))
									{
										...
										$user_details_data = $this->dbadminheader->user_details_for_edit($id);
...
<div  id="save_cancle" style="float:left; margin-left:6px;"  ><a onclick="document.getElementById('fetch_user_data').value='';document.getElementById('quote').style.visibility='hidden';document.getElementById('fade1').style.display='none'" href="javascript:void(0)" class="quote-close">Cancel</a>
		...
		//$totalRows= $this->dbadminheader->user_details_state_for_email($state);
		...
		//$totalRows= $this->dbadminheader->count_user_search($username,$member_id);
		$totalRows= $this->dbadminheader->count_user_search_email($username,$member_id,$id,$mm_type);
		...
		$config['base_url'] = base_url().'admincp/email/edit';
