...
class User extends CI_Controller {
	...
	public function more_user()
	{
		...
		$chapter_id = $this->session->userdata('get_chapter_id');
			...
			if($member_type == '0')
			{
				$num_rec=$this->dbuser->count_user_search_final($chapter_id,$member_search);
			}
			
			if($member_type == '1')
			{
				$num_rec=$this->dbuser->count_user_search_verified_final($chapter_id,$member_search);				
			}
			
			if($member_type == '2')
			{
				$num_rec=$this->dbuser->count_user_search_unverified_final($chapter_id,$member_search);
		...
		$config['base_url'] = base_url().'chapteradmincp/user/more_user/';
...

		{
			...
			if($member_type == '0')
			{
				$data['query'] = $this->dbuser->get_all_user_search_final($chapter_id,$member_search,$config['per_page'],$segment);
			}
			if($member_type == '1')
			{
				$data['query'] = $this->dbuser->get_all_user_search_verified_final($chapter_id,$member_search,$config['per_page'],$segment);
				//var_dump($data['query']);
				
			}
			if($member_type == '2')
			{
				$data['query'] = $this->dbuser->get_all_user_search_unverified_final($chapter_id,$member_search,$config['per_page'],$segment);
			}
