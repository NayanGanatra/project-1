...
class Form extends CI_Controller
{
	...
	public function edit($id)
	{
		...
		else
		{
			...
			$result=$this->dbform->edit($data,$id);
				...
				redirect(base_url().'admincp_convention/form');
		...
		$this->load->view('admincp_convention/form/edit',$data);
	...
	public function add()
	{
		...
		$this->load->view('admincp_convention/form/add',$data);
	}
	/*public function view1()
	{
		...
		$config['base_url'] = base_url().'admincp_convention/form/view/';
		...
		$this->load->view('admincp_convention/form/view',$data);
	...
	public function view()
	{
		...
		{
			$search_key = 0;
			$config['base_url'] = base_url().'admincp_convention/form/view/'.$search_key.'/';
		}
		else
		{
			$config['base_url'] = base_url().'admincp_convention/form/view/'.$search_key.'/';
...

	{
		...
		redirect(base_url().'admincp_convention/form/view');
	...
	function ExportToCSV_edit($content,$filename = '', $filedArr = '',$heading)
	{
		...
		//user details
		$chapter = $this->dbform->get_chapter_from_mm_id($program_deatails->mm_id);
		
		$ch_name = $chapter->ch_name;
		$mm_address = $chapter->mm_address;
		$mm_fname = $chapter->mm_fname;
		$mm_life_id = $chapter->mm_life_id;
		$mm_lname = $chapter->mm_lname;
		$mm_hphone = $chapter->mm_hphone;
		$mm_cphone = $chapter->mm_cphone;
		$mm_email = $chapter->mm_email;
		
		
		$get_cities = $this->dbform->cities($chapter->mm_city_id);
		$city_name = $get_cities->city_name;
		$get_state = $this->dbform->state($chapter->mm_state_id);
