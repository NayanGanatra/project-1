	...
	for (var i=0;i<document.templates.elements.length;i++)
	{
		...
		if (e.type=='checkbox')
		{
			if(e.checked)
			{
				...
				if(x==true)
				{
					document.templates.action = "<?php echo base_url();?>admincp/templates/activeall/";
	...
	for (var i=0;i<document.templates.elements.length;i++)
	{
		...
		if (e.type=='checkbox')
		{
			if(e.checked)
			{
				...
				if(x==true)
				{
					document.templates.action = "<?php echo base_url();?>admincp/templates/deactiveall/";
					document.templates.submit();
					return true;
				}
				else
	...
	for (var i=0;i<document.templates.elements.length;i++)
	{
		...
		if (e.type=='checkbox')
		{
			if(e.checked)
			{
				...
				if(x==true)
				{
					document.templates.action = "<?php echo base_url();?>admincp/templates/deleteall/";
					document.templates.submit();
					return true;
				}
				else
...
if ($query)
{
   ...
   foreach ($query as $row)
   {
       ...
       <?php
		   ...
		   {
			   ?>
			   <a href="<?php echo base_url();?>admincp/templates/active/<?php echo $row->templates_id; ?>" title='Active'><img src="<?php echo base_url();?>images/approve_icon.gif" border="0" title='Active' alt="Active" /></a> 
			   <?php
		   }
		   else
		   {
			   ?>
			   <a href="<?php echo base_url();?>admincp/templates/deactive/<?php echo $row->templates_id; ?>" title='Deactive'><img src="<?php echo base_url();?>images/disapprove_icon.gif" border="0" title='Deactive' alt="Deactive" /></a> 
			   <?php
		   }
	   ?>
       <a href="<?php echo base_url();?>admincp/templates/delete/<?php echo $row->templates_id; ?>" title='Delete' onclick="javascript: return del();"><img src="<?php echo base_url();?>images/delete_16.gif" border="0" title='Delete' alt="Delete" /></a></span>
        </td>
        <td width="10"></td>
        <?php   
		}
