...
class User extends CI_Controller {
	...
	public function view()
	{
		...
		$config['base_url'] = base_url().'chapteradmincp/user/view/';
	...
	function passreset()
	{
		...
		if ($this->form_validation->run() == TRUE)
		{
			...
			else
			{
				...
				for($i=0; $i < count($mem_ids); $i++)
				{
					...
					$this->email->message($this->parser->parse('chapteradmincp/forgot_parser', $final_message , TRUE));
		...
		$data['description'] = "";
		$data['keywords'] = "";
		$this->load->view('chapteradmincp/user/passreset',$data);
	}
