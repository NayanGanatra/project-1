...
class Verification extends CI_Controller {
...

	{
		...
		$this->load->model('chapteradmincp/dbverification');
...

	{
		...
		$config['base_url'] = base_url().'chapteradmincp/verification/';
		...
		$this->load->view('chapteradmincp/verification/view',$data);
...

	{
...

		{
				...
				redirect(base_url().'chapteradmincp/verification');

		}

		$data['title'] = 'Verification Email';		

		$this->load->view('chapteradmincp/verification/add',$data);
...

	{
		...
		if(!$data['item']){redirect(base_url().'chapteradmincp/verification');}
		...
		$this->load->view('chapteradmincp/verification/edit',$data);
		...
		if($just_use == '0' || $just_use == 0)
		{
			if($fetch_user_data=="" || $fetch_user_data=='0')
			{
				...
				
				{
					/*if(isset($_POST['edit']))
					{
						$user_details_data = $this->dbadminheader->user_details_for_edit_verification($id);
