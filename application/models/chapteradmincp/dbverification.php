...
class Dbverification extends CI_Model
{
	...
	*/
	function count_verification()
	{
		...
		$sql="SELECT b.* FROM verification a, chapter_to_verification_template b WHERE a.uid = b.email_template_id AND b.ch_id = '".$this->session->userdata('get_chapter_id')."'";
		$query = $this->db->query($sql);
	...
	function get_all_verification($num,$offset)
	{
		...
		$sql="SELECT a.* FROM  verification a,chapter_to_verification_template b WHERE  a.uid = b.email_template_id AND b.ch_id = '".$this->session->userdata('get_chapter_id')."' ORDER BY a.uid DESC LIMIT ".$offset.", ".$num."";
		$query = $this->db->query($sql);
		return $query->result();
	}
