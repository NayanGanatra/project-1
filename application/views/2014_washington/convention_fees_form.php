<?php 
	...
	if($data['item'])
	{
	    ...
	    $this->load->view($this->config->item('convention_folder').'convention_fees_view',$data);
		...
		if($data['reg_status']>0)
		{
			...
			$this->load->view($this->config->item('convention_folder').'convention_fees_edit',$data);
		}
		else
		{
			$this->load->view($this->config->item('convention_folder').'convention_fees_add',$data);
		}
	}
	else
	{ ?>
