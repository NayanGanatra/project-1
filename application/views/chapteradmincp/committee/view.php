        ...
        <?php
...

			{
...

				{
                    ...
                    <h4><?php echo $get_chapters_row->ch_name; ?> Committee Members <span style="float:right; font-size:14px;">

                    <?php 

						$chapter_id = $this->session->userdata('get_chapter_id');

						if($chapter_id == $get_chapters_row->ch_id)

						{

					?>

                    <a href="<?php echo base_url('chapteradmincp/committee/add'); ?>">Add Member</a>
...

                                {
                                    ...
                                    ?>
                                        ...
                                        <?php 
											...
											if($chapter_id == $row->cm_ch_id)

											{

										?>

                                        <td><a title="Edit" href="<?php echo base_url('chapteradmincp/committee/edit/'.$row->cm_id.'/'.friendlyURL($row->mm_fname).'.html');?>"><i class="icon-edit"></i></a> <a onclick="javascript: return del();" title="Delete"  href="<?php echo base_url('chapteradmincp/committee/delete/'.$row->cm_id.'/'.$row->cm_mm_id.'/'.friendlyURL($row->mm_fname).'.html');?>"><i class="icon-minus-sign"></i></a></td>
