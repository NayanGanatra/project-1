...
<table border="0" cellspacing="2" cellpadding="2" width="100%" class="table table-hover">
    ...
    <tr>
        ...
        <td><a href="<?php echo base_url(); ?>admincp_convention_texas/program/view"/><?php echo $program; ?></a></td>
        <td><a href="<?php echo base_url(); ?>admincp_convention_texas/medical/view"/><?php echo $medical; ?></a></td>
        <td><a href="<?php echo base_url(); ?>admincp_convention_texas/form/edit/<?php echo $row->fm_id; ?>"/><?php echo $attend; ?></a></td>
        <td><?php $total = 0;
		foreach($subtotal_of_reg as $subtotal){ echo $subtotal->fm_total_fee; 
		$total += $subtotal->fm_total_fee;
		}?></td>
        <td><a href="<?php echo base_url(); ?>admincp_convention_texas/event_member/view"/><?php echo $event; ?></a></td>
        <td><?php foreach($subtotal_of_event as $subtotal_event){ echo $subtotal_event->ce_total; 
		$total += $subtotal_event->ce_total;
		} ?></td>
        <td><?php echo $total; ?></td>
        <?php /*?><td><a href="<?php echo base_url(); ?>admincp_convention_texas/form/edit/<?php echo $row->fm_id; ?>" title="<?php echo $this->lang->line('misc_edit');?>"><i class="icon-edit"></i></a>
        <a href="<?php echo base_url();?>admincp_convention_texas/form/delete/<?php echo $row->fm_id; ?>" title='<?php echo $this->lang->line('misc_delete');?>' onclick="javascript: return del();"><i class="icon-minus-sign"></i></a></td><?php */?>
    </tr>
	<?php
   }
}
...
{
	var id = $id;
	//alert('http://localhost/spcsusa/admincp_convention_texas/view'+id);
	document.getElementById('myform').action = BASE_URI+'admincp_convention_texas/convention_detail/view/'+id;
	document.getElementById('myform').submit();
			
			/*$.ajax({
				   type: "POST",
			
			url: BASE_URI+"admincp_convention_texas/convention_detail/show",
			data :'mm_id='+id,
