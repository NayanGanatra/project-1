...
$('#newsletter_template_name').change(function() {
	$.ajax({
	  url: BASE_URI+'admincp/user/get_template_chapter/'+$('#newsletter_template_name').val(),
...
function chkalluser(uid,type)
{
	...
	if(status=='yes')
	{
		...
		if(id!='')
		{
			...
			$.ajax({
				   type: "POST",
			url: BASE_URI+"admincp/user/user_check_uncheck_all/add/"+uid+"/true/"+type+"/"+$('#offset').val(),
				success: function(data) {
					if(type!='chkall')
					{
...
$('div#custom_box').removeClass("custom_box_css");
	...
	else
	{
		...
		if(id!='')
		{
			...
			url: BASE_URI+"admincp/user/user_check_uncheck_all/add/"+uid+"/false/"+type+"/"+$('#offset').val(),
				success: function(data) {
...
$('#email_template_name').change(function() {
	$.ajax({
	  url: BASE_URI+'admincp/user/get_template_chapter/'+$('#email_template_name').val(),
	  success: function(data) {
		$('.nicEdit-main').html(data);
		$('#html').html(data);
		}
...
$('#cnt_user').click(function() {
	...
	$.ajax({
		type: "POST",
	 	...
	 	//url: BASE_URI+"admincp/user/get_template_user/"+username+"/0/"+<?php echo $item->uid; ?>+"/0/"+fetch_user_data+"/"+$('#check_uncheck_all_user_status').val(),
		...
		url: BASE_URI+"admincp/user/get_template_user/",
	   	//url: BASE_URI+"admincp/user/get_template_user/"+username+"/0/"+<?php echo $item->uid; ?>+"/0/"+$('#check_uncheck_all_user_status').val(),
	  	success: function(data) {
		  var str=data.split("|");
...
function user_check_uncheck(uid,m_id,action)
{
	...
	$.ajax({
				   type: "POST",
	url: BASE_URI+"admincp/user/user_check_uncheck/edit/"+uid+"/"+m_id+"/"+action,
	success: function(data) {
		if(action==true)
		{
...
$('#save_user_data').click(function() {
	...
	for (var i = 0, l = ln_user; i < l; i++) {
		...
		$.ajax({
				   type: "POST",
		  url: BASE_URI+"admincp/user/edit_user",
		  success: function(data) {
...
function chkall_d()
{
	...
	$.ajax({
		type: "POST",
	 	url: BASE_URI+'admincp/user/check_uncheck_all_user/'+$('#chkall_check').val(),
	  	success: function(data) {
		  var str=data.split("|");
...
$('#check_uncheck_all_user').click(function() {
	...
	$.ajax({
			type: "POST",
	  url: BASE_URI+'admincp/user/check_uncheck_all_user/'+$('#check_uncheck_all_user_status').val(),
	  success: function(data) {
		  var str=data.split("|");
...
$('#email_preview').click(function() {
	...
	$.ajax({
		   type: "POST",
		   url: BASE_URI+"admincp/user/email_info/",
		   //$('#html').html(data);
