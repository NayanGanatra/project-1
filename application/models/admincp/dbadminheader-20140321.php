...
function buildtree($id,$title,$islink)
 ...
 { 
	...
	$querySUB="SELECT page_id,page_sub_id,page_title,page_menu_name,page_seo,page_order,page_status,islink,page_link FROM pages WHERE type=0 AND page_sub_id=$id order by page_order";
...
WHERE a.template_id='".$template_id."' AND d.ch_id='".$arr_ch_id."' AND e.mm_status='1' AND e.mm_varify='1'";
...
AND e.mm_status = '1'";
...

	{
		 ...
		 $sql="select * from chapter_to_email_template where email_template_id = '".$email_template_id."' AND ch_id = '".$ch_id."' LIMIT 1";
...

	{
		...
		$this->db->delete('chapter_to_verification_template', array('email_template_id' => $id));
...

	{
		...
		$sql="select * from chapter_to_verification_template where email_template_id = '".$template_id."' AND ch_id = '".$ch_id."'";
...

	{
		...
		$this->db->delete('verification_template_to_member', array('email_template_id' => $id));
...

	{
		...
		$sql="select * from chapter_to_newsletters where newsletter_id =

		'".$uid."' AND ch_id = '".$ch_id."' LIMIT 1";
	...
	function get_chapters_newsletter($newsletter_id)
	...
	function get_newsletters_subscribe($id)

	{

		$sql = "SELECT a . * , b . *

		FROM newsletters a, newsletters_subscribe b

		WHERE a.uid = '".$id."'

		AND a.uid = b.newsletter_id

		ORDER BY b.subscribe_id ASC";
...

	{
	 ...
	 echo $sql="SELECT * FROM member_master WHERE FIND_IN_SET(mm_id,'".$mm_id_set."') AND mm_id='".$mm_id."' AND mm_varify='1' AND mm_status='1'  ORDER BY mm_id ASC LIMIT 1";
...
*/
///////////pradip_20140314////////
	function user_details_pagination_with_check_all($mm_id,$m_type)

	{
		...
		if($m_type=='1')
		{
			$sql="SELECT * FROM member_master WHERE FIND_IN_SET(mm_id,'".$mm_id."') AND mm_family_id = '0' ORDER BY mm_id ASC";		
		}
		if($m_type=='2')
		{
			//$sql="SELECT * FROM member_master WHERE FIND_IN_SET(mm_id,'".$mm_id."') AND mm_varify='1' AND mm_status='1' ORDER BY mm_id ASC";
			$sql="SELECT a.*,b.* FROM member_master a, states b,chapter_to_state c WHERE  b.state_id = c.state_id AND a.mm_state_id = b.state_id   AND FIND_IN_SET(a.mm_id,'".$mm_id."') ORDER BY mm_id ASC";
		}
		if($m_type=='3')
		{
			//$sql="SELECT * FROM member_master WHERE FIND_IN_SET(mm_id,'".$mm_id."') AND mm_varify='1' AND mm_status='1' ORDER BY mm_id ASC";
			$sql="SELECT a.* FROM member_master a WHERE  a.mm_state_id NOT IN (SELECT state_id FROM chapter_to_state GROUP BY state_id )  AND  FIND_IN_SET(a.mm_id,'".$member_id."') ORDER BY mm_id ASC";
		}
		if($m_type=='4')
		{
			//$sql="SELECT * FROM member_master WHERE FIND_IN_SET(mm_id,'".$mm_id."') AND mm_varify='1' AND mm_status='1' ORDER BY mm_id ASC";	
			$sql="SELECT * FROM member_master WHERE   FIND_IN_SET(mm_id,'".$member_id."') AND mm_varify='1' ORDER BY mm_id ASC";
		}
		
		if($m_type=='4')
		{
			//$sql="SELECT * FROM member_master WHERE FIND_IN_SET(mm_id,'".$mm_id."') AND mm_varify='1' AND mm_status='1' ORDER BY mm_id ASC";	
			$sql="SELECT * FROM member_master WHERE   FIND_IN_SET(mm_id,'".$member_id."') AND mm_varify='0' ORDER BY mm_id ASC";
		}
	...
	////////////////////end/////////////////////
	function user_details_pagination_with_check_all_page($mm_id,$offset,$num)
...

	{
		...
		$this->db->query("DELETE FROM events_to_member WHERE email_template_id ='".$id."'");
	...
	//update end
	//////////////////////////////////////////////pradip changes for event 20130806////////////////
	...
	function fetch_event_ch_name($mm_state_id,$ch_id)

	{

$sql="SELECT b.ch_name FROM chapter_to_state a, chapters b WHERE a.state_id = '".$mm_state_id."' AND b.ch_id = '".$ch_id."' LIMIT 1";
	...
	function count_user_search_event($search_key,$member_id,$event_id,$mm_type)
...
if($mm_type==0)
	...
	function get_all_user_search_event($search_key,$offset,$num,$member_id,$event_id,$mm_type)
	{
		if($mm_type==0)
		{
			if($search_key=='0')
...

	{
		...
		$this->db->delete('chapter_to_pages', array('page_id' => $id));
	...
	function fetch_user_for_approve_comment($bid)

	{

	$sql="SELECT * FROM blog_comment WHERE `blog_id` ='".$bid."' AND verify=0";
