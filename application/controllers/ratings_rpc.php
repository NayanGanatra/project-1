...
class Ratings_rpc extends CI_Controller {
	function __construct()
	{
		parent::__construct();
		 $this->load->model('dbratings');
    ...
    function index()
    {
        ...
        if ($numbers = $this->dbratings->findBy_id($id_sent))
        {
        ...
        if ($this->dbratings->countBy_ip($ip, $id_sent) == 0)
        {            
            //make sure vote is valid and IP matches - no monkey business!
            if ($vote_sent > 0 && $ip == $ip_num) 
            { 
                $this->dbratings->updateBy_id($id_sent, array(
                    'total_votes' => $added,
                    'total_value' => $sum,
