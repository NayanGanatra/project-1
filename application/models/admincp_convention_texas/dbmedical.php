...

{
...

	{
		...
		if($this->db->update($this->config->item('convention_texas_db_prefix').'medical_form', $data))
...

	{
		...
		$sql ="SELECT a.* FROM ".$this->config->item('convention_texas_db_prefix')."medical_form a WHERE a.md_mm_id IN (SELECT mm_id from member_master WHERE mm_fname like '%".$search_key."%' OR mm_lname like '%".$search_key."%') OR md_name like '%".$search_key."%' OR md_cellphone like '%".$search_key."%' OR md_home_phone like '%".$search_key."%' ORDER BY md_id DESC";
...

	{
		...
		$sql ="SELECT a.* FROM ".$this->config->item('convention_texas_db_prefix')."medical_form a WHERE a.md_mm_id IN (SELECT mm_id from member_master WHERE mm_fname like '%".$search_key."%' OR mm_lname like '%".$search_key."%') OR md_name like '%".$search_key."%' OR md_cellphone like '%".$search_key."%' OR md_home_phone like '%".$search_key."%' ORDER BY md_id DESC LIMIT ".$offset.", ".$num."";
...

	{
		...
		$sql="SELECT * FROM ".$this->config->item('convention_texas_db_prefix')."medical_form WHERE md_id = '$id'";
...

	{
		...
		$this->db->delete($this->config->item('convention_texas_db_prefix').'medical_form', array('md_id' => $id));
...

	{
		...
		$sql ="SELECT * FROM ".$this->config->item('convention_texas_db_prefix')."medical_form WHERE md_id = '".$id."' LIMIT 1";
